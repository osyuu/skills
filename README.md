...
| `release-assets` | 把使用者會看到的非程式碼交付物交乾淨:release notes、商店截圖、changelog、版本號;先盤點、讀懂要取代的東西、用讀者身分審、清殘留 |
| `comment-budget` | 註解量的 warn-only pre-commit 守門:過長區塊、單檔註解佔比、敘事/驗證過程字眼;與 arch-guard、sdd-harness-init 共用同一個 hook |

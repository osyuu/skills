...
- **三層載入,把東西放對層**:metadata(name + description)**永遠在 context**、約 100 字;SKILL.md body 觸發時才載入、**目標 <120 行**(本 repo 實際族群 74–193 行;`<500` 一次都不會開火,而完全不開火等於沒裝);`references/`、`assets/`、`scripts/` 按需載入。不是每次都要的往下層放,body 只留一句「何時去讀它」。body 逼近 120 行 → 拆層,別硬塞。
...
3. **push 完要手動 `/plugin update`,開新 session 不會自動拉。** 2026-08-28 實測(Claude Code 2.1.247):push 之後開 headless session、開互動式新 session,60 秒內 `installed_plugins.json` 的 `gitCommitSha` 都沒動;跑 `/plugin update` 之後約 32 秒才更新。追的是 git HEAD 不是 `metadata.version`,所以 `version` bump 只是給人讀的變更標記,不是觸發條件。**別假設別台會自己拿到新版**——它們會一路用舊版工作,而兩邊都沒有訊號。
...
7. **`disable-model-invocation: true`** 用在「內容是流程順序、不是某個請求的解法」的 skill 上。
   2026-08-28 實測語意:它讓該 skill **退出模型的 skill 清單**(不再佔清單預算),**且模型用
   Skill 工具也叫不動**(harness 擋下來並回「請使用者自己跑 `/<name>`」)。使用者的 slash
   command 路徑保留。要連使用者也叫不到是 `user-invocable: false`,那是另一個旗標。
...
**這是一個 stage,不是一道閘門**:報告收到、逐條有結論(修掉／接受不修＋理由)就交付。
要不要再跑一輪,答 `dev-loop`〈要不要再跑一次〉那兩行。
...
乾淨」「量基準」——三個當初寫下來都很合理。判準寫得好長什麼樣,看 `comment-budget` 的〈門檻〉與 `claim-check` 的〈校準〉——**它們都寫下了自己的開火率**,那才是可判定的形狀。

## 品質自檢(寫完/改完自問)

- description 有沒有同時講「做什麼 + 何時用」?夠不夠 pushy?要不要加負面觸發?
- 清單裡這個 skill 有帶出描述嗎?**空白先查 `skillUsage` 分數與清單預算,別直接怪 YAML**(見上節)。
- body 超過 120 行了嗎?**要加一條之前先刪一條**——每條規則加進去的當下都有一個真實踩坑當理由,所以永遠沒有拒絕的理由,規則庫只會單調遞增。每次都要的才留 body,其餘進 references/assets。

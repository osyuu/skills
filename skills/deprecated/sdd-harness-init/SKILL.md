...
1. **打勾的任務，驗收項還沒打勾**——認兩種 spec 形狀：任務行標 ✅ 且指名 `AC<n>`、
   該 AC 仍是 `- [ ]`（逐條關聯，零誤判）；或 spec 沒有 AC-ID（`design-doc` 模板刻意
   不引 requirement ID）時退為檔案級——任務全數勾銷、驗收節仍有未勾項。檔案級的任務
   證據認三種：任務節 checkbox、任務表列的 ✅（`| T1 |…`，含 ✅＝完成、不含＝未完）、
   同名 `<slug>.tasks.md`。**沒有任何完成標記的 spec 不評**——沒有宣稱就沒有 drift，
   代價是「做完但從不標記」的 repo 這條看不見。混用格式（部分 AC 有 ID）時整檔歸
   逐條關聯管，沒 ID 的項不在射程。

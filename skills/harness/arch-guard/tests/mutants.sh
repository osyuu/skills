...
mut "checker 對沒有規則可跑的 conf 出聲" "assets/arch-guard-check.sh" "s.replace('&& [ \"\$chk_rules\" -eq 0 ]; then', '&& false; then')"
mut "註解不算有效的 chokepoint"       "assets/arch-guard-check.sh" "s.replace('grep -cv', 'grep -c')"
mut "只有 chokepoint 的 conf 不被誤殺" "assets/arch-guard-check.sh" "s.replace('[ \"\$chk_rules\" -eq 0 ]', 'true')"

...
mut "疑問句不算 hedge"          "s.replace('if QUESTION.search(clause):', 'if False:')"
...
mut "commit 的受詞要收斂"       "s.replace('|the (change|changes|fix|fixes|branch|PR|commit|patch))', '|the [a-z]+)')"
mut "LGTM 不分大小寫"           "s.replace(r'|(?i:\\bLGTM\\b)', r'|LGTM')"

sec "── 兩類 hedge 的作用範圍（合成一類就會二選一地壞掉）──"
mut "否定類跨子句"              "s.replace('HEDGE_NEG.search(text[neg_from:at])', 'HEDGE_NEG.search(text[clause_start:at])')"
mut "條件類不跨子句"            "s.replace('HEDGE_COND.search(text[clause_start:at])', 'HEDGE_COND.search(text[sent_start:at])')"
mut "轉折之後重新起算"          "s.replace('for m in CONTRAST.finditer(text[sent_start:at]):', 'for m in []:')"
mut "疑問判定降到子句"          "s.replace('if QUESTION.search(clause):', 'if QUESTION.search(text[sent_start:sent_end.end() if sent_end else len(text)]):')"

sec "── hedge 詞表本身 ──"
mut "英文收條件與計畫標記"      "s.replace('if|unless|when|until|whenever|whether|before', 'before')"
mut "cannot 要單獨收"           "s.replace('|cannot|can ?not', '')"
mut "中文收否定不只收條件"      "s.replace('|沒有|沒能|不是|還沒|尚未|無法|未能|並未|不會', '')"
mut "嗎/呢 也算疑問"            "s.replace('[?？]\\\\s*$|嗎|呢', '[?？]\\\\s*$')"
...
sec "── 放寬的受詞要收斂 ──"
mut "pushed to 的受詞"          "s.replace('pushed to (origin|upstream|main|master|develop|HEAD', 'pushed to (\\\\S+|origin|upstream|main|master|develop|HEAD')"
mut "commit 的受詞不含 work"    "s.replace('|the (change|changes|fix|fixes|branch|PR|commit|patch))', '|the (change|changes|fix|fixes|branch|PR|commit|patch|work))')"

...
**這三條大多自己答得出來，答得出來就別問。** 語言看副檔名、分層看有沒有多層目錄、
既有守門看 `hooks/` 與 `.claude/settings.json`——step 1 已經掃過了。把掃得到的東西
轉成問句，是把自己的工作轉嫁給使用者，而使用者的答案還不會比掃描結果準。

**真的要問的時候，先給推薦答案，讓使用者一個字接受。**「要裝 arch-guard 嗎？」是在
要求對方重跑一次你的判斷；「這個 repo 有 3 層目錄，建議裝 arch-guard（可以嗎）」
只要一個「好」。分歧真的存在時才補一行說明分歧在哪；step 1 已經定案的項目直接跳過，
連提都不用提。
...
**這個 repo 若有決策記錄，同時留一筆**（`DECISIONS.md`、ADR 目錄、或任何等價的
翻案記錄——沒有就跳過，別為了留這筆去裝一套）。理由不是形式：

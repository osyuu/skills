...
1. **打勾的任務，驗收項還沒打勾**——任務表標 ✅，它指名的 AC 仍是 `- [ ]`。純機械、零誤判。
   ⚠️ **已知缺口：這條會跟著 `SPEC_SRC_DIRS` 一起死。** checker 在建索引前有一道
   `[ -z "$SRC" ] && exit 0`，而這條規則**只讀 spec 裡的打勾狀態、根本不需要原始碼目錄**——
   路徑沒填或填錯時它照樣被那道 exit 帶走，整支靜默結束。也就是說**三條規則裡零誤判的那條，
   正好是最容易在沒人發現的情況下失效的**。務必照流程 2 注入故障驗一次。未修。

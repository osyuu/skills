...
# 快取檔要帶 uid 與 cwd：全機共用一個檔時，兩個 session 在不同 repo 會互蓋，
# 5 秒內顯示對方的分支——錯得像對的。共用主機上還會撞別人的檔而寫不進去。
GIT_CACHE_MAX_AGE=5
_gc_key=$(printf '%s' "${cwd_full:-$PWD}" | cksum | cut -d' ' -f1)
GIT_CACHE="${TMPDIR:-/tmp}/claude-statusline-$(id -u)-${_gc_key}"
...
git_cache_is_stale() {
  ...
  local cache_age=$(( $(date +%s) - $(stat -f %m "$GIT_CACHE" 2>/dev/null || stat -c %Y "$GIT_CACHE" 2>/dev/null || echo 0) ))

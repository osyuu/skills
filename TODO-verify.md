...
- **P1-2 `.done` 擋不住部分失敗。** 它只證明 driver 跑到最後一行。有 **41 個 id
...
## 第四輪外審 —— `review-r4-product`

9 項發現（3 P1、6 P2）。**它用真實語料推翻了我上一輪採納的決定**：

- **P1-1 `HEDGE_NEG` 跨子句是負收益。** 同一份 27 檔語料新舊各跑一次：抑制率
  5.5% → **8.7%**（+58%），7 次跨子句抑制裡 2 次是錯的，而**它要抓的形狀在語料裡
  一個自然實例都沒有——只出現在討論它自己的設計文件裡**。
  修法不是整條退回：它給的兩個條件合起來有更精確的切法——**否定只跨逗號、且只有
  英文跨**。中文的「A,B」幾乎都是兩個獨立子句（`並未動到 parser,測試全綠`），
  英文的 `I have not, as you asked, committed…` 才是被逗號切開的同一個子句。
  分號、冒號、頓號一律不跨。七個案例因此全部分開。
- **P1-2 疑問詞沒有錨**：`嗎|呢` 對整個子句比對、含命中之後的文字，於是
  「測試全綠 還要我再跑一次嗎」整條被吃掉，而加個逗號就開火——**差別只在標點**。
  改成要求「命中之後只剩那個語氣詞」，並拿掉 `呢`（非疑問用法更常見）。
- **P1-3 `wrote|writes` 與同一顆 commit 排除 `after`/`once` 的理由自相矛盾**
  （都是完成態標記），且無測試無注入守護。拿掉。
- P2：`pushed to` 兩個方向都錯（誤中 `logs/app.log`、漏抓沒有斜線的分支名），
  且 `RE_GIT` 不含 `git push`——這顆 commit 把 push 提升成一級宣稱形狀而證據那側
  沒跟上；受詞拿掉 `work` 方向選錯（誤中比漏抓罕見）；`beside` **打穿了
  `CLAIM_CHECK_HOME` 的隔離**（與 GIT_DIR 同形），改成只在沒有明確覆寫時才找；
  `sent_end` 是死變數，而依賴它的那條注入是被 `NameError` 殺掉的——**被 crash
  殺掉的 mutation 沒有在測任何東西**；註解合併把 `said` 的排除理由誤植成「完成態」。

它另外量出 14 條注入是「組層級有守護者、逐字沒有」，以及四個常數裡 `呢`、
CONTRAST 中文半、CLAUSE_END 的 `、`、`_conf` 的 `beside` 是真正的裸點。已補測試。

測試 110→120、注入 53→58。真實語料回放 finding 23→23（不變，與「那個形狀沒有
自然實例」一致）。

## 第四輪外審 —— `review-r4-harness`

1 個 P1 ＋ 4 個 P2。**它用我第三輪的方法找到了同一類的第三個實例**：

- **P1 `format/fix 排除唯讀形式` 那條注入是靠 `SyntaxError` 轉紅的**。它用
  `s.index('"(?![^')` 切字串，抓到的是 `r` **後面**那個雙引號，切完留下一個裸的 `r`
  → checker 連編譯都過不了。它殺掉 51 條斷言、其中 49 條輸出含 Traceback，
  而真正守那段行為的**只有 1 條**。後果不是誤紅，是**它永遠會紅，所以永遠偵測不到
  自己守護者的消失**——那段 code 的突變覆蓋率實際上是 0。
- **它提的偵測器已經常駐進六支**：注入之後對目標檔做語法檢查（`.py` 走 `ast.parse`、
  `.sh` 走 `sh -n`），壞掉就判「注入讓目標檔語法壞掉 —— 它是靠 crash 轉紅的」。
  這把「紅了但理由不對」從 reviewer 才找得到的東西變成機械檢查。裝完當場抓到那條 P1。
  （它另外提了一個近乎免費的分診信號：**指紋的殺傷集合基數分佈，離群值就是嫌犯**。）
- **P2-1 第三種「exit 0 但沒跑完」**：`MUTANT_JOBS` 非數字時 `$((seq_n % JOBS))`
  在 `set -u` 下當場打死整支，只印區塊標題、沒有摘要行、**exit 0**，pre-commit 照樣
  印綠勾（平台相依：dash 下是 exit 2）。**檔尾的完成性斷言接不住這條**（根本走不到），
  所以改用 EXIT trap 的完成哨兵——那蓋得到任何一種提早死。
- P2-2 `hooks/mutants.sh` 是 28 條注入 ÷ JOBS 14 = 0 餘數，所以「刪掉結尾 `wait`」
  這個驗證在**那一支**根本觸發不了（最後一條 `mut` 已經 wait 過）。不是修法壞了，
  是驗證方法在該支無效——要用 `MUTANT_JOBS=8` 才驗得到。
- P2-3 「31 個 id 只被 `no` 型消費」是上一顆的數字，現在是 **41**（盲區大 32%）。已改。
- P2-4 `.missing` 用 `[ -f ]` 擋不了「存在但為空」，而 `probe()` 在 `fn` 拋例外時
  會寫出空檔。目前不可達（41 個 no-only id 全部由 `rep()` 產出），但改成 `[ -s ]`
  零成本且嚴格更強。已改。

它逐條實證的四項「攻不破」：`.missing` 的子 shell 可見性／冪等／trap 時機／
`break` 語意等價；`continue` 不造成無窮迴圈；指紋合約重產後五支逐 byte 相同、
claim-check 的差異剛好由 13 條新注入解釋；環境洩漏四項比對無變化。

**一處更正**：`repo 層的 conf` 那條不是「import 期 TypeError」，`_conf()` 是函式，
錯誤發生在**呼叫**時，型別是 `TypeError: 'PosixPath' object is not iterable`。其餘解讀證實。

**修的過程中自己又踩一次**：偵測器裝進五支之後腳本在 claim-check 拋錯，前兩步算了
但沒寫入，我只手動補了第三步——於是 claim-check 有完成性斷言、**沒有偵測器**，
而它照樣印 58/0。跑一次才發現。**裝一道不會開火的守門比不裝更糟。**

**reviewer 關機前補的一點,已採納**:偵測器自己也需要守門——它靜默失效的話,
「靠 crash 轉紅」會悄悄回來,而那正是它裝進來要擋的東西。六支各加一條**自檢**:
送一個必定壞語法的注入(`)(`,python 與 sh 都是語法錯誤)進去,它必須指認得出來。
用序號 0、不動 `seq_n`,所以不影響完成性斷言的計數。**注入證明過**:把六支的
偵測器停用之後,自檢全部攔下來並 exit 非零。
